 ...
 */
export const SAMPLE_TEST_PROMPTS = [
    {
 ...
 */
export class AITestingQuickStart {
    
    constructor(private testingService: AIModelTestingService) {}

    async showQuickStartWizard(): Promise<void> {
    ...
    private async runSampleTest(): Promise<void> {
        const selectedSample = await vscode.window.showQuickPick(
            SAMPLE_TEST_PROMPTS.map(sample => ({
        ...
        if (selectedSample) {
            await this.testingService.runComparativeTest(selectedSample.sample.prompt, {
                includeContext: true,
                category: selectedSample.sample.category.toLowerCase() as any,
                saveResults: true
            });
        }
    }
    ...
    private async runSelectedCodeTest(): Promise<void> {
    ...
    private async showTestingGuide(): Promise<void> {
        // Open the testing guide
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (workspaceFolders) {
            const guideUri = vscode.Uri.joinPath(
                workspaceFolders[0].uri, 
                'vscode-extension/claude-portfolio/src/docs/AI_TESTING_GUIDE.md'
            );
     ...
     */
    async showTestingTips(): Promise<void> {
        const tips = `
# 🧪 AI Testing Pro Tips
