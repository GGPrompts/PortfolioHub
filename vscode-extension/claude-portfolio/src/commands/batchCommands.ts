...
export class BatchCommands {
     ...
     */
    private async killProcessOnPortFallback(port: number): Promise<boolean> {
        return new Promise((resolve) => {
            const { spawn } = require('child_process');
     ...
     */
    private async batchAddToWorkspaceCommand(): Promise<void> {
        try {
            const selectedProjects = this.projectProvider.getSelectedProjectsData();
            ...
            if (!await VSCodeSecurityService.requireWorkspaceTrust('Add projects to workspace')) {
                return;
            }
            ...
            if (this.configService.isBatchOperationConfirmationEnabled()) {
                const confirmation = await vscode.window.showInformationMessage(
                    `Add ${selectedProjects.length} selected project(s) to workspace?`,
                    { modal: true },
                    'Yes', 'No'
                );
            ...
            await vscode.window.withProgress({
                ...
                for (let i = 0; i < selectedProjects.length; i++) {
                    ...
                    try {
                        ...
                        // Sanitize path
                        const sanitizedPath = await VSCodeSecurityService.sanitizePath(
                            projectPath, 
                            portfolioPath
                        );
     ...
     */
    private async batchNpmInstallCommand(): Promise<void> {
        try {
            const selectedProjects = this.projectProvider.getSelectedProjectsData();
            ...
            // Check workspace trust first
            if (!await VSCodeSecurityService.requireWorkspaceTrust('Run npm install on projects')) {
                return;
            }
            ...
            if (this.configService.isBatchOperationConfirmationEnabled()) {
                const confirmation = await vscode.window.showInformationMessage(
                    `Run npm install on ${selectedProjects.length} selected project(s)?`,
                    { modal: true },
                    'Yes', 'No'
                );
     ...
     */
    private async batchNpmBuildCommand(): Promise<void> {
        try {
            const selectedProjects = this.projectProvider.getSelectedProjectsData();
            ...
            // Check workspace trust first
            if (!await VSCodeSecurityService.requireWorkspaceTrust('Run npm build on projects')) {
                return;
            }
            ...
            if (this.configService.isBatchOperationConfirmationEnabled()) {
                const confirmation = await vscode.window.showInformationMessage(
                    `Run npm run build on ${selectedProjects.length} selected project(s)?`,
                    { modal: true },
                    'Yes', 'No'
                );
     ...
     */
    private async batchNpmTestCommand(): Promise<void> {
        try {
            const selectedProjects = this.projectProvider.getSelectedProjectsData();
            ...
            // Check workspace trust first
            if (!await VSCodeSecurityService.requireWorkspaceTrust('Run npm test on projects')) {
                return;
            }
            ...
            if (this.configService.isBatchOperationConfirmationEnabled()) {
                const confirmation = await vscode.window.showInformationMessage(
                    `Run npm test on ${selectedProjects.length} selected project(s)?`,
                    { modal: true },
                    'Yes', 'No'
                );
     ...
     */
    private async batchGitStatusCommand(): Promise<void> {
        try {
            const selectedProjects = this.projectProvider.getSelectedProjectsData();
            ...
            // Check workspace trust first
            if (!await VSCodeSecurityService.requireWorkspaceTrust('Check git status on projects')) {
                return;
            }
            ...
            if (this.configService.isBatchOperationConfirmationEnabled()) {
                const confirmation = await vscode.window.showInformationMessage(
                    `Check git status on ${selectedProjects.length} selected project(s)?`,
                    { modal: true },
                    'Yes', 'No'
                );
     ...
     */
    private async batchGitPullCommand(): Promise<void> {
        try {
            const selectedProjects = this.projectProvider.getSelectedProjectsData();
            ...
            // Check workspace trust first
            if (!await VSCodeSecurityService.requireWorkspaceTrust('Run git pull on projects')) {
                return;
            }

            // Show confirmation if enabled - always confirm for git pull as it modifies repositories
            const confirmation = await vscode.window.showWarningMessage(
                `Run git pull on ${selectedProjects.length} selected project(s)? This will update all repositories.`,
                { modal: true },
                'Yes', 'No'
            );
