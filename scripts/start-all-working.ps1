# Working PowerShell script to start all projects
param(
    [switch]$NoPortfolio,
...
function Start-Project {
    ...
    # Create the command with proper PowerShell syntax
    $fullCommand = @"
cd '$Path'
...
foreach ($name in $projectsToStart.Keys) {
    ...
    if (Test-Path $project.Path) {
        ...
        Write-Host "⚠️ Project $name path not found: $($project.Path)" -ForegroundColor Red
    }
}
