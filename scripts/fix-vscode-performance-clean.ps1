...
if (-not $isAdmin) {
    Write-Host "WARNING: Running without admin privileges - some optimizations may be skipped" -ForegroundColor Yellow
}

# 1. Clean up old VSIX files (saves 84MB and stops file watching)
Write-Host "`n[1/7] Removing old extension packages..." -ForegroundColor Yellow
...
if ($packageCount -gt 0) {
    ...
    Write-Host "SUCCESS: Removed $packageCount old packages (saved $([math]::Round($packageSize, 2)) MB)" -ForegroundColor Green
} else {
    Write-Host "SUCCESS: No old packages to remove" -ForegroundColor Green
}

# 2. Clean build artifacts
Write-Host "`n[2/7] Cleaning build artifacts..." -ForegroundColor Yellow
$dirsToClean = @(
...
foreach ($dir in $dirsToClean) {
    if (Test-Path $dir) {
        try {
            ...
            Write-Host "SUCCESS: Cleaned: $(Split-Path $dir -Leaf)" -ForegroundColor Green
            $cleanedCount++
        } catch {
            Write-Host "WARNING: Could not clean: $(Split-Path $dir -Leaf)" -ForegroundColor Yellow
        }
    }
}
Write-Host "SUCCESS: Cleaned $cleanedCount directories" -ForegroundColor Green

# 3. Kill any hanging Node/TypeScript processes
Write-Host "`n[3/7] Checking for hanging processes..." -ForegroundColor Yellow
...
if ($killedCount -gt 0) {
    Write-Host "SUCCESS: Killed $killedCount hanging processes" -ForegroundColor Green
} else {
    Write-Host "SUCCESS: No hanging processes found" -ForegroundColor Green
}

# 4. Add Windows Defender exclusion (requires admin)
if ($isAdmin) {
    Write-Host "`n[4/7] Adding Windows Defender exclusion..." -ForegroundColor Yellow
    try {
        ...
        if ($exclusions -notcontains "D:\ClaudeWindows\claude-dev-portfolio") {
            ...
            Write-Host "SUCCESS: Added Defender exclusion for better performance" -ForegroundColor Green
        } else {
            Write-Host "SUCCESS: Defender exclusion already exists" -ForegroundColor Green
        }
    } catch {
        Write-Host "WARNING: Could not add Defender exclusion: $_" -ForegroundColor Yellow
    }
} else {
    Write-Host "`n[4/7] Skipping Defender exclusion (requires admin)" -ForegroundColor Yellow
}

# 5. Create optimized VS Code settings
Write-Host "`n[5/7] Optimizing VS Code settings..." -ForegroundColor Yellow
...
if (Test-Path $settingsPath) {
    ...
    Write-Host "SUCCESS: Backed up current settings to settings.backup.json" -ForegroundColor Green
}
...
Write-Host "SUCCESS: Applied performance-optimized settings" -ForegroundColor Green

# 6. Clear TypeScript/VS Code caches
Write-Host "`n[6/7] Clearing editor caches..." -ForegroundColor Yellow
$cachePaths = @(
...
if ($clearedCache) {
    Write-Host "SUCCESS: Cleared old cache files" -ForegroundColor Green
}

# 7. Show running Node processes for awareness
Write-Host "`n[7/7] Current Node.js processes:" -ForegroundColor Yellow
...
if (-not $isAdmin) {
    Write-Host "`nTip: Run as Administrator for additional optimizations" -ForegroundColor Yellow
}
