...
import { VSCodeSecurityService } from '../src/securityService';
import { SecureCommandRunner } from '../../../src/services/securityService';
...
export class SecurityTestRunner {
     ...
     */
    private async testCommandsShouldPass(): Promise<void> {
        ...
        for (const command of COMMANDS_SHOULD_PASS) {
            try {
                ...
                const vsCodeResult = VSCodeSecurityService.validateCommand(command);
                const reactResult = SecureCommandRunner.validateCommand(command);
     ...
     */
    private async testCommandsShouldFail(): Promise<void> {
     ...
     */
    private async testMessagePassingSecurity(): Promise<void> {
        ...
        for (const command of MESSAGE_PASSING_BYPASS_ATTEMPTS) {
            try {
                ...
                const reactResult = SecureCommandRunner.validateCommand(command);
                const reactEnhanced = SecureCommandRunner.validateCommandEnhanced(command);
     ...
     * Test 4: Path traversal protection
     */
    private async testPathTraversalProtection(): Promise<void> {
        console.log('🛡️ Testing Path Traversal Protection...');

        for (const test of PATH_TRAVERSAL_TESTS) {
            try {
                // Test VS Code path sanitization
                const sanitizedPath = await VSCodeSecurityService.sanitizePath(test.input, test.workspaceRoot);
     ...
     * Test 5: Enhanced error messages
     */
    private async testEnhancedErrorMessages(): Promise<void> {
        ...
        for (const test of ERROR_MESSAGE_TESTS) {
            try {
                ...
                const vsCodeResult = VSCodeSecurityService.validateCommandEnhanced(test.command);
                const reactResult = SecureCommandRunner.validateCommandEnhanced(test.command);
    ...
    private generateSummaryReport(): void {
        ...
        const groupedResults = this.results.reduce((groups, result) => {
            ...
            return groups;
        }, {} as Record<string, { passed: number; failed: number; results: TestResult[] }>);
 ...
 */
export function quickSecurityCheck(): void {
    ...
    criticalTests.forEach(test => {
        const vsCodeResult = VSCodeSecurityService.validateCommand(test.cmd);
        const reactResult = SecureCommandRunner.validateCommand(test.cmd);
