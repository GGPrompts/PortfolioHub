...
class PortfolioWebviewProvider {
     ...
     */
    async _loadOrganizedNotes(projectId) {
        try {
            let notesPath;
     ...
     */
    async _readNotesRecursively(uri, basePath) {
        const notes = [];
        try {
     ...
     */
    _parseNoteFile(content, fileName, relativePath) {
        try {
    ...
    async _startVSCodeServer(serverType = 'vscode-web', port = 8080) {
        try {
            ...
            if (success) {
                ...
                setTimeout(async () => {
                    const choice = await vscode.window.showInformationMessage('VS Code Server should be ready now. Open it in Edge browser?', 'Open Edge', 'Later');
                    if (choice === 'Open Edge') {
                        try {
...
<body class="vscode-webview-integration">
    ...
    async _openLivePreview(url, title, projectId) {
        try {
            ...
            if (livePreviewExtension && livePreviewExtension.isActive) {
                ...
                await vscode.env.openExternal(vscode.Uri.parse(url));
                vscode.window.showInformationMessage(`Opened ${title} in Edge browser. Install Live Preview extension for better experience.`, 'Install Live Preview').then(selection => {
     ...
     */
    async _refreshAndInjectProjectData() {
        console.log('🔄 Refreshing and injecting fresh project data');
        // Force reload of project data with fresh status
        this._cachedProjectData = null;
        const freshData = await this._loadProjectDataWithStatus();
        this._cachedProjectData = freshData;
        console.log('🔄 Fresh project data loaded:', freshData.projects?.map((p) => `${p.id}: ${p.status}`));
    ...
    async _refreshProjectStatus() {
        ...
        if (this._cachedProjectData?.projects) {
            ...
            console.log('🔄 Project statuses refreshed:', this._cachedProjectData.projects?.map((p) => `${p.id}: ${p.status}`));
