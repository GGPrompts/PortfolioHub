...
class WebSocketBridgeService {
    ...
    async handleProjectLaunchAll(id) {
        try {
            const workspaceRoot = path.join(this.portfolioPath, '..');
            const command = 'cd D:\\ClaudeWindows\\claude-dev-portfolio; .\\scripts\\start-all-enhanced.ps1';
            const success = await securityService_1.VSCodeSecurityService.executeSecureCommand(command, 'Launch All Projects', workspaceRoot);
            return {
                id,
                success,
    ...
    async handleProjectKillAll(id) {
        try {
            const workspaceRoot = path.join(this.portfolioPath, '..');
            const command = 'cd D:\\ClaudeWindows\\claude-dev-portfolio; .\\scripts\\kill-all-servers.ps1';
            const success = await securityService_1.VSCodeSecurityService.executeSecureCommand(command, 'Kill All Projects', workspaceRoot);
            return {
                id,
                success,
    ...
    async handleEnhancedProjectLaunch(id, data) {
        try {
            ...
            const success = await securityService_1.VSCodeSecurityService.executeSecureCommand(command, 'Enhanced Project Launch', workspaceRoot);
            return {
                id,
                success,
    ...
    async handleProjectStatusSync(id) {
        try {
    ...
    async handleBatchAddToWorkspace(id, projectIds) {
        try {
    ...
    async handleBatchNpmInstall(id, projectIds) {
        try {
            if (!projectIds || projectIds.length === 0) {
                return {
                    id,
    ...
    async handleBatchNpmBuild(id, projectIds) {
        try {
            if (!projectIds || projectIds.length === 0) {
                return {
                    id,
    ...
    async handleBatchNpmTest(id, projectIds) {
        try {
            if (!projectIds || projectIds.length === 0) {
                return {
                    id,
    ...
    async handleBatchGitStatus(id, projectIds) {
        try {
            if (!projectIds || projectIds.length === 0) {
                return {
                    id,
    ...
    async handleBatchGitPull(id, projectIds) {
        try {
            if (!projectIds || projectIds.length === 0) {
                return {
                    id,
